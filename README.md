...
- ヘルスチェックAPIの作成
- ミッション一覧取得APIの作成
- Reactからfetchを使用したAPI呼び出し
- APIから取得したミッションの初期表示
- Reactからのアクセスを許可するCORS設定
- HTTPエラーと通信エラーの基本的な処理

現在はミッション一覧を取得するGET通信まで実装しています。

APIのミッションデータはJavaコード内に直接記述しており、
データベースにはまだ保存していません。

また、追加・編集・削除はフロントエンド側のみの処理で、
画面を再読み込みするとAPIから取得したデータで上書きされます。

次の段階では、ミッション追加・編集・削除APIの作成と、
フロントエンドの各操作との接続を予定しています。
